# 🛩️ Sistema de Gerenciamento de Agenda e Aulas — Aeroclube Asas Abertas

**Protótipo desenvolvido no RiseUp 2026.1 | Porto Digital + NTTDATA via Residência em Software & Inteligência Artificial** em parceria com a Faculdade Senac Recife-PE.

Plataforma web que digitaliza a gestão administrativa do Aeroclube Asas Abertas (PE), substituindo papéis e planilhas por um sistema centralizado de cadastros, agendamentos, diário de aulas e controle financeiro — com foco em segurança operacional.

![Introdução — Aeroclube Asas Abertas](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Aeroclub%20Asas%20Abertas.png)

---

## 🎯 Problema

O Aeroclube Asas Abertas gerenciava alunos, professores e agendamentos de voo de forma manual — papéis e planilhas.  
Sem controle digital, surgiam conflitos de horário, informações perdidas e falhas de comunicação que comprometem a operação e a segurança de voo.

Problemas relatados:
- Dois alunos reservam o mesmo instrutor ou aeronave no mesmo horário; ✖️  
- Rasuras e folhas extraviadas impedem rastrear cancelamentos e presenças; 🗂️  
- Cobranças duplicadas e pagamentos não registrados geram prejuízo financeiro; 💸  
- Descontos aplicados errado causam desgaste com o aluno; ⚠️  
- Aeronaves alugadas duas vezes para o mesmo horário, descoberto só na pista; 🛬  
- Professor comparece sem aluno por falha de comunicação; 📣  
- Aluno chega e descobre a aeronave em manutenção sem aviso prévio; 🔧  

Em aviação, falha de informação vai além do administrativo — compromete a **segurança**.

![Problema](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Problema.png)

---

## ✅ A Solução

A plataforma proposta digitaliza e automatiza regras operacionais para reduzir erros e aumentar a segurança:

- Agendamentos digitais eliminam conflitos de horário 🔁  
- Controle de acesso por perfil protege os dados 🔐  
- Diário digital registra presenças, notas e conteúdo 📘  
- Aluno acompanha seu histórico em tempo real 📊  
- Sistema bloqueia cancelamentos fora do prazo ⏳  
- Inadimplentes são impedidos de agendar aulas 🚫  
- Alunos sem CMA são bloqueados automaticamente 🛡️  
- Cadastro centralizado substitui planilhas isoladas 🗃️  
- Painel inicial reúne métricas e agendamentos 🎛️  
- Histórico de aulas acessível a qualquer momento 📚  
- Professor aprova ou reprova cancelamentos registrados ✅  
- Cores indicam disponibilidade de horários na agenda 🎨  

---

## 🖼️ Protótipos

### Visão do Aluno
![Protótipo da parte do Aluno](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Prot%C3%B3tipo%20Solu%C3%A7%C3%A3o%20Web%20Aluno.png)

### Visão do Professor
![Protótipo da parte do Professor](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Prot%C3%B3tipo%20Solu%C3%A7%C3%A3o%20Web%20Instrutor.png)

### QR Code / Acesso rápido ao protótipo
![QR Code do protótipo](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Squad%20e%20Prot%C3%B3tipo.png)

---

## 🌍 Impacto ESG

### **E – Environmental**
- Eliminação do uso de papel na gestão administrativa. 🌱

### **S – Social**
- Digitalização de escola de aviação civil, ampliando acesso organizado à formação de pilotos. 👥

### **G – Governance**
- Digitalização com rastreabilidade, padronização e controle de dados, apoiando compliance e segurança operacional. 📋

---

## 📈 Mercado

**Organização é Prioridade!**  
Aeroclubes e escolas de aviação civil no Brasil enfrentam crescimento da demanda por pilotos civis e necessidade crescente de digitalização operacional.

Contexto: regulamentação da ANAC e exigências do Ministério da Aeronáutica criam obrigações de registro que sistemas manuais não suportam com segurança.

**Benchmark:** o projeto não apresenta benchmark formal.  
Como referência implícita, o sistema foi validado com usuários reais do Aeroclube Coroa do Avião (PE), que opera com as mesmas dores. Sistemas similares existem em escolas de aviação de grande porte, mas são inacessíveis ou inapropriados para aeroclubes de médio porte.

---

## 💼 Modelo de Negócio

Protótipo desenvolvido como projeto acadêmico em parceria com a NTTData via Residência em Software & Inteligência Artificial (Porto Digital / Faculdade Senac Recife-PE).

O modelo de negócio futuro pressupõe licenciamento da plataforma para aeroclubes, com backend, banco de dados e módulos financeiro, de manutenção e documentos ainda em desenvolvimento.

**Digitalizar Evita Conflito e Perda de Informação** ✈️

![Modelo de Negócios](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Modelo%20de%20Neg%C3%B3cios.png)

---

## 🧾 Análise de Custo e Viabilidade

- **220 horas totais**
- **Custo total:** R$ 4.700,00
- **Prazo estimado:** aproximadamente 12 semanas

![Análise de Custo e Viabilidade](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/An%C3%A1lise%20de%20Custo%20e%20Viabilidade.png)

---

## 🎯 Público-Alvo

- Alunos aspirantes a pilotos, por exemplo, Piloto Privado de Avião ✈️  
- Instrutores de voo habilitados 👨‍✈️  
- Administradores de aeroclubes 🧑‍💼  
- Gestores de manutenção e operações 🔧  

---

## 🛠️ Tecnologias Utilizadas

- 💻 **HTML5**
- 🎨 **CSS3**
- 🧠 **JavaScript**
- 🖥️ **Figma**
- 🚀 **Vercel**
- 📅 **Scrum**
- 🤝 **Porto Digital**
- 🏢 **NTTData**
- 🏢 **Faculdade Senac Recife-PE**

---

## 🚀 Como Executar Localmente

1. Clone o repositório:
```bash
git clone https://github.com/thaynabds/AeroclubAsasAbertas.git
```

2. Acesse a pasta do projeto:
```bash
cd AeroclubAsasAbertas
```

3. Abra o arquivo `index.html` no navegador.

**Observação:** projeto 100% frontend, sem necessidade de backend.

---

## 👥 Equipe — Squad 43

| Nome | Função / Responsabilidades |
|------|---------------------------|
| **Thayná Batista da Silva** | Scrum Master · Análise de Requisitos · Product Designer · Design e Prototipação (Figma) |
| Henrique Nascimento Vasco Alves | Líder de Teste do sistema; Relatos de Inteligência Artificial; Análise de custo e de viabilidade |
| James Bernardino Da Silva Neto | Responsividade do menu lateral; Relatos de Inteligência Artificial; Análise de custo e de viabilidade |
| Pedro Henrique Marques da Silva | Responsividade geral de todo o site; Front-End (HTML, CSS e JavaScript); Relatos de Inteligência Artificial; Análise de custo e de viabilidade |
| Pedro Lucas de Souza Pessoa | Líder de Programação Front-End: HTML, CSS e JavaScript; Repositório do código do sistema; Relatos de Inteligência Artificial; Análise de custo e de viabilidade; Líder de Regras de Negócio |
| Pedro Ruan Verissimo da Costa | Front-End (HTML, CSS e JavaScript); Relatos de Inteligência Artificial; Análise de custo e de viabilidade |
| Silvoney Felipe Bernardino | Auxílio da Pesquisa e Documentação; Revisão da documentação; Líder de Documentação: Formatação e organização de documentação; Relatos de Inteligência Artificial |
| Sophia Maria Lima da Costa | Produção da Documentação; Revisão da documentação; Pesquisa de Tipografia e esquema de cores para o sistema; Relatos de Inteligência Artificial; Regras de negócio |
| Suelen Bezerra Alves dos Santos | Relatos de Inteligência Artificial; Produção da Documentação; Desenvolvimento; Responsividade do site; Líder de Análise de custo e de viabilidade |
| Tatiana Cristina Nunes Bezerra | Líder de Pesquisa; Aplicação e Produção do Questionário de pesquisa; Pesquisa de casos e de Requisitos; Personas; Trilha inicial; Comparativos de funcionalidades web x mobile; Relatos de Inteligência Artificial |
| Vicente de Paula Gomes da Silva Junior | Prototipagem inicial de baixa fidelidade no Figma; Trilha inicial; Levantamento dos Requisitos Funcionais e Não Funcionais; Regras de negócios; Líder de Criação de Fluxo de telas; Participação na Documentação; Teste do sistema pronto |

---

## 📽️ Demonstrações

- Protótipo funcional: https://asasabertas-aero-clube.vercel.app/  
- Vídeo do protótipo: [Assista aqui](https://lnkd.in/dZ5u4z6Z) 🎬  
- Pitch Deck: [Assista ao Pitch](https://lnkd.in/dhV3dBw7) 📊  

---

## 📬 Contato

<div align="center">

  <a href="https://br.linkedin.com/in/thaynabds" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  
  <a href="https://www.instagram.com/thaynabdstec/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>

</div>

📧 E-mail: [thaynabdstec@gmail.com](mailto:thaynabdstec@gmail.com)  
📱 Telefone: +55 (81) 97912-6121

<div align="center">

![Cartão TEC Thayná](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Cart%C3%A3o%20TEC%20Thayn%C3%A1%20Batista%20da%20Silva.png)

</div>

---

# 🇬🇧 English Version

# 🛩️ Schedule and Lessons Management System — Aeroclube Asas Abertas

**Prototype developed in RiseUp 2026.1 | Porto Digital + NTTData through Software & Artificial Intelligence Residency** in partnership with Senac College Recife-PE. 

Web platform that digitizes administrative management for Aeroclube Asas Abertas in Pernambuco, replacing paper and spreadsheets with a centralized system for registrations, scheduling, lesson logs, and financial control — with a strong focus on operational safety.

![Introduction — Aeroclube Asas Abertas](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Aeroclub%20Asas%20Abertas.png)

---

## 🎯 Problem

Aeroclube Asas Abertas managed students, instructors, and flight bookings manually using paper and spreadsheets.  
Without digital control, scheduling conflicts, lost information, and communication failures compromise operations and flight safety.

Reported issues:
- Two students book the same instructor or aircraft at the same time; ✖️  
- Erasures and lost sheets prevent tracking cancellations and attendance; 🗂️  
- Duplicate charges and unregistered payments generate financial loss; 💸  
- Incorrect discounts cause student dissatisfaction; ⚠️  
- Aircraft are booked twice for the same time and discovered only on the runway; 🛬  
- The instructor arrives without a student due to communication failure; 📣  
- The student arrives and finds the aircraft under maintenance without prior notice; 🔧  

In aviation, information failure goes beyond administration — it affects **safety**.

![Problem](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Problema.png)

---

## ✅ Solution

The proposed platform digitizes and automates operational rules to reduce errors and increase safety:

- Digital scheduling eliminates time conflicts 🔁  
- Access control by profile protects data 🔐  
- Digital logbook records attendance, grades, and content 📘  
- Students can track their history in real time 📊  
- The system blocks late cancellations ⏳  
- Delinquent students are prevented from booking lessons 🚫  
- Students without CMA are automatically blocked 🛡️  
- Centralized registration replaces isolated spreadsheets 🗃️  
- The dashboard gathers metrics and bookings 🎛️  
- Lesson history is always available 📚  
- Instructors approve or reject cancellations ✅  
- Colors indicate schedule availability 🎨  

---

## 🖼️ Prototypes

### Student View
![Student Prototype](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Prot%C3%B3tipo%20Solu%C3%A7%C3%A3o%20Web%20Aluno.png)

### Instructor View
![Instructor Prototype](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Prot%C3%B3tipo%20Solu%C3%A7%C3%A3o%20Web%20Instrutor.png)

### QR Code / Quick Access
![Prototype QR Code](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Squad%20e%20Prot%C3%B3tipo.png)

---

## 🌍 ESG Impact

### **E – Environmental**
- Eliminates paper use in administrative management. 🌱

### **S – Social**
- Digitalization improves organized access to civil aviation training. 👥

### **G – Governance**
- Centralized and traceable records support compliance and operational safety. 📋

---

## 📈 Market

Organization is a priority.  
Aeroclubs and civil aviation schools in Brazil face growing demand for pilots and a rising need for digital operations.

Context: ANAC regulations create record-keeping obligations that manual systems cannot safely support.

**Benchmark:** no formal benchmark.  
The system was validated with real users from Aeroclube Coroa do Avião (PE), which has the same operational pain points. Similar systems exist in large aviation schools, but they are often inaccessible or unsuitable for medium-sized aeroclubs.

---

## 💼 Business Model

This prototype was developed as an academic project in partnership with NTTData via the Software & Artificial Intelligence Residency (Porto Digital / Senac Recife-PE Faculty).

The future business model involves licensing the platform to aeroclubs, with backend, database, and financial, maintenance, and document modules still under development.

**Digitizing Prevents Conflicts and Information Loss** ✈️

![Business Model](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/Modelo%20de%20Neg%C3%B3cios.png)

---

## 🧾 Cost and Feasibility Analysis

- **220 total hours**
- **Total cost:** R$ 4.700,00
- **Estimated timeline:** approximately 12 weeks

![Cost and Feasibility](https://raw.githubusercontent.com/thaynabds/AeroclubAsasAbertas/refs/heads/main/An%C3%A1lise%20de%20Custo%20e%20Viabilidade.png)

---

## 🎯 Target Audience

- Student pilots, such as Private Pilot candidates ✈️  
- Certified flight instructors 👨‍✈️  
- Aeroclub administrators 🧑‍💼  
- Maintenance and operations managers 🔧  

---

## 🛠️ Technologies Used

- 💻 **HTML5**
- 🎨 **CSS3**
- 🧠 **JavaScript**
- 🖥️ **Figma**
- 🚀 **Vercel**
- 📅 **Scrum**
- 🤝 **Porto Digital**
- 🏢 **NTTData**
- 🏢 **Faculdade Senac Recife-PE**

---

## 🚀 Run Locally

1. Clone the repository:
```bash
git clone https://github.com/thaynabds/AeroclubAsasAbertas.git
```

2. Go to the project folder:
```bash
cd AeroclubAsasAbertas
```

3. Open the `index.html` file in your browser.

**Note:** this is a 100% frontend project and does not require a backend.

---

## 👥 Team — Squad 43

| Name | Role / Responsibilities |
|------|--------------------------|
| **Thayná Batista da Silva** | Scrum Master · Requirements Analysis · Product Designer · Design and Prototyping (Figma) |
| Henrique Nascimento Vasco Alves | System Testing Lead; Artificial Intelligence reports; cost and feasibility analysis |
| James Bernardino Da Silva Neto | Sidebar responsiveness; Artificial Intelligence reports; cost and feasibility analysis |
| Pedro Henrique Marques da Silva | General site responsiveness; Front-End (HTML, CSS and JavaScript); Artificial Intelligence reports; cost and feasibility analysis |
| Pedro Lucas de Souza Pessoa | Front-End Programming Lead: HTML, CSS and JavaScript; system code repository; Artificial Intelligence reports; cost and feasibility analysis; Business Rules Lead |
| Pedro Ruan Verissimo da Costa | Front-End (HTML, CSS and JavaScript); Artificial Intelligence reports; cost and feasibility analysis |
| Silvoney Felipe Bernardino | Research and Documentation Support; documentation review; Documentation Lead: formatting and organization; Artificial Intelligence reports |
| Sophia Maria Lima da Costa | Documentation production; documentation review; typography and color scheme research; Artificial Intelligence reports; business rules |
| Suelen Bezerra Alves dos Santos | Artificial Intelligence reports; documentation production; development; site responsiveness; cost and feasibility analysis lead |
| Tatiana Cristina Nunes Bezerra | Research Lead; questionnaire application and production; case and requirements research; personas; initial flow; web vs mobile feature comparisons; Artificial Intelligence reports |
| Vicente de Paula Gomes da Silva Junior | Initial low-fidelity prototyping in Figma; initial flow; functional and non-functional requirements; business rules; screen flow creation lead; documentation participation; final system testing |

---

## 📽️ Demonstrations

- Live prototype: https://asasabertas-aero-clube.vercel.app/  
- Prototype video: [Watch here](https://lnkd.in/dZ5u4z6Z) 🎬  
- Pitch Deck: [Watch the Pitch](https://lnkd.in/dhV3dBw7) 📊  

---

## 📬 Contact

<div align="center">

  <a href="https://br.linkedin.com/in/thaynabds" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  
  <a href="https://www.instagram.com/thaynabdstec/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>

</div>

📧 Email: [thaynabdstec@gmail.com](mailto:thaynabdstec@gmail.com)  
📱 Phone: +55 (81) 97912-6121

<div align="center">

![TEC Card Thayná](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Cart%C3%A3o%20TEC%20Thayn%C3%A1%20Batista%20da%20Silva.png)

</div>
